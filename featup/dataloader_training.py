...
class SpinalCordDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        image_slices = np.load(image_path)['arr_0'].reshape(-1, 384, 224, 224)  # Shape: (H, W, D)
        image_slices = np.transpose(image_slices, (3, 2, 1, 0))
        label_slices = np.load(label_path)['arr_0'].reshape(-1, 1, 224, 224)  # Shape: (H, W, D)
        label_slices = np.transpose(label_slices, (3, 2, 1, 0))
        ...
        # print(image_slices.shape[2])

        for i in range(image_slices.shape[3]):
            ...
            lbl_tensor = torch.tensor(lbl_slice, dtype=torch.float32).permute(2, 0, 1)    # Shape: (1, H, W)
        ...
        transformed_images = torch.stack(transformed_images, dim=0)  # Shape: (20, 3, H, W)
        transformed_labels = torch.stack(transformed_labels, dim=0)  # Shape: (20, 1, H, W)

        # print("TRANSFORMED IMAGES: ", transformed_images.shape)
